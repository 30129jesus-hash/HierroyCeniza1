...
import { SEASON_DAYS } from '../game/premium';
...
function load(): MetaState {
  try {
    ...
    const base = { ...defaultState(), ...parsed, collection: { ...STARTER_COLLECTION, ...(parsed.collection ?? {}) } };
    if (!base.seasonEnds || base.seasonEnds < Date.now()) {
      base.seasonEnds = Date.now() + SEASON_DAYS * 86400000;
    }
    return base;
  } catch {
    return defaultState();
...
type Action =
  ...
  | { type: 'claimOffer'; date: string; card: string }
  | { type: 'toggleMute' }
  | { type: 'toggleMusic' }
  | { type: 'reset' };
...
function reducer(s: MetaState, a: Action): MetaState {
  switch (a.type) {
    ...
    case 'spendDiamonds':
      return { ...s, diamonds: Math.max(0, s.diamonds - a.amount) };
    case 'unlockFrame':
    ...
    case 'claimOffer':
      return { ...s, offerClaimedDate: a.date, gold: s.gold + 1000, collection: { ...s.collection, [a.card]: (s.collection[a.card] ?? 0) + 1 } };
    case 'toggleMute':
