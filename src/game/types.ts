...
export interface SpellEffect {
  school: SpellSchool;
  amount: number;
  amount2?: number; // duración de veneno / daño mejorado, etc.
  target: SpellTarget;
}
...
export interface CardDef {
  ...
  swift?: boolean;   // veloz: ataca la ronda en que se despliega
  thorns?: number;   // espinas: daño devuelto al atacante
  ...
  art?: string;      // URL/ruta de ilustración real (opcional)
}
...
export interface UnitInst {
  ...
  swift: boolean;
  thorns: number;
  poisonAtk: number;   // veneno al golpear
  ...
  fresh: boolean;    // desplegada esta ronda: no ataca (salvo Veloz)
}
...
export interface BattleConfig {
  ...
  enemyHeroHp?: number;    // vida del jefe (pactos oscuros)
}
...
export interface BattleState {
  ...
  baseDecks: { player: CardDef[]; enemy: CardDef[] }; // pozo para rearmar el mazo
  ...
  logSeq: number;
  uidSeq: number;
  __pendingLog?: BattleEvent[]; // interno: eventos de la operación en curso
  kills: number;
...
export interface MetaState {
  ...
  relicsOwned: string[];   // relicario hueco
  seasonEnds: number;      // timestamp de fin de temporada de marcos limitados
  offerClaimedDate: string;
  muted: boolean;
  musicOn: boolean;
}
