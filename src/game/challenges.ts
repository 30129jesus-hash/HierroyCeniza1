...
/* Información del contador de un desafío semanal (si la tiene). */
export const weeklyCounter = (id: string): { counter: string; need: number } | undefined => {
  const def = WEEKLY_POOL.find((c) => c.id === id);
  return def && def.counter && def.need !== undefined ? { counter: def.counter, need: def.need } : undefined;
};
