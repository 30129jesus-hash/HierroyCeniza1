...
import type { BattleConfig, MetaState, RelicDef } from './game/types';
...
import {
  ArsenalHub, BattleHub, FeatsHub, MarketHub, RelicPicker, TitleScreen,
...
function shuffleDefs<T>(arr: T[]): T[] {
  const a = [...arr];
...
/* baraja 20 cartas al azar de un mazo enemigo base */
function buildBattleDeck(poolIds: string[]): import('./game/types').CardDef[] {
  ...
  return shuffleDefs(deck).slice(0, 20);
}
...
function playerBattleDeck(collection: Record<string, number>, saved: string[] = []): import('./game/types').CardDef[] {
  if (saved.length === 20 && saved.every((id) => ALL_CARDS[id])) {
    return shuffleDefs(saved.map(cardById));
  }
...
function Inner() {
  ...
  const [relicPick, setRelicPick] = useState<{ options: RelicDef[]; nextStreak: number } | null>(null);
  const [achToast, setAchToast] = useState<{ id: string; at: number }[]>([]);
  const [chToast, setChToast] = useState<{ id: string; at: number }[]>([]);

  const go = (name: Screen['name'], tab?: string) => {
    if (tab) setTabs((p) => ({ ...p, [name]: tab }));
  ...
  const startStory = (level: number, pacts: string[] = []) => {
    ...
    const bonus = lv.bonus + (pacts.includes('pacto_ceniza') ? 1 : 0);
    const energy = pacts.includes('pacto_hambre') ? 1 : 0;
    const cfg: BattleConfig = {
      ...
      heroHp, maxRounds: 20,
      enemyStatBonus: bonus, enemyEnergyBonus: energy,
    };
  ...
  const grantAch = (proj: ProjectedMeta, stats: BattleStats, won: boolean) => {
    ...
    earned.forEach((id, i) => {
      setTimeout(() => {
        setAchToast((t) => [...t, { id, at: Date.now() }]);
        sfx.epic();
      }, 600 + i * 1800);
      setTimeout(() => {
        setAchToast((t) => t.filter((x) => x.id !== id));
      }, 600 + i * 1800 + 4000);
    });
  };

  const resolveCh = (outcome: BattleOutcome) => {
    const res = resolveChallenges(meta, outcome);
    if (res.claimedIds.length === 0 && Object.keys(res.counters).length === 0) return;
    dispatch({ type: 'challengesResolve', claimedIds: res.claimedIds, gold: res.gold, counters: res.counters });
    res.claimedIds.forEach((id, i) => {
      setTimeout(() => {
        setChToast((t) => [...t, { id, at: Date.now() }]);
        sfx.coin();
      }, 1200 + i * 1600);
      setTimeout(() => {
        setChToast((t) => t.filter((x) => x.id !== id));
      }, 1200 + i * 1600 + 4000);
    });
  };
  ...
  const onBattleEnd = (won: boolean, stats: BattleStats, conceded: boolean) => {
    ...
    if (!battleCtx) return;
    const { mode, param, pacts } = battleCtx;
    ...
    const chOutcome: BattleOutcome = {
      mode, won, conceded,
      diff: param,
      newStreak: mode === 'supervivencia' && won && !conceded ? streak + 1 : (mode === 'supervivencia' ? 0 : streak),
      kills: stats.kills,
    ...
    if (mode === 'historia') {
      ...
      if (won && !conceded) {
        ...
        const pactMult = pacts.reduce((m, id) => m * pactById(id).mult, 1);
        const gold = Math.round((first ? lv.reward : 25) * pactMult);
        ...
        setReward({
          ...
          lines: [
            ...
            pacts.length > 0 ? `Los Pactos Oscuros multiplican tu botín (×${pactMult.toFixed(2).replace(/\.?0+$/, '')}).` : '',
          ...
          {chToast.map((t) => {
            ...
            return (
              <div key={t.at} className="panel-dark px-5 py-3 flex items-center gap-3 anim-slide-down" style={{ borderColor: `hsl(${c.hue} 60% 45% / 0.7)` }}>
                ...
                <div>
                  <p className="font-body text-[0.58rem] uppercase tracking-widest text-blood-400">Desafío superado</p>
