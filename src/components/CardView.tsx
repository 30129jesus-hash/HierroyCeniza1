...
const SCHOOL_ICON: Record<string, string> = {
  ...
  ashRain: 'fire', spikeShield: 'shield', shadowDagger: 'dagger', warCry: 'banner',
  forbidden: 'book', destinyArrow: 'bow',
};
...
export default function CardView({ card, size = 'hand', playable = true, selected = false, dimmed = false, count, onClick }: Props) {
  ...
  return (
    ...
    >
      {/* halo del marco cosmético equipado (capa interior, no pisa la rareza) */}
      {frame && (
      ...
      <div
        ...
        style={{
          ...
          boxShadow: selected
            ...
            : `0 6px 18px rgba(0,0,0,0.6), inset 0 0 14px rgba(0,0,0,0.65)${rarHalo > 0 ? `, inset 0 0 10px ${rarGlow}` : ''}`,
        }}
      />
      ...
      <div className="absolute inset-x-2 top-6 sm:top-7 bottom-[38%] flex items-center justify-center overflow-hidden"
        ...
        {card.ranged && isUnit && (
          <div className="absolute top-0.5 left-1 text-gold-400 z-10" title="A distancia: puede atacar al héroe">
            <Sigil icon="bow" className="w-3 h-3" />
          </div>
        )}
        <div className="absolute bottom-0.5 right-1 w-1.5 h-1.5 rotate-45 z-10" style={{ background: rarC, boxShadow: `0 0 6px ${rarC}` }} />
        {count !== undefined && count > 1 && (
          <div className="absolute top-0.5 right-1 font-display text-xs text-bone-300 bg-ink-900/80 px-1 leading-tight z-10">×{count}</div>
        )}
      </div>

      {/* nombre */}
      <div className="absolute inset-x-1 bottom-[30%] text-center leading-none z-10">
      ...
      {/* texto */}
      <div className="absolute inset-x-1.5 bottom-6 sm:bottom-7 top-[72%] overflow-hidden z-10">
        <p className="font-body text-[0.5rem] sm:text-[0.58rem] leading-[1.15] text-bone-300/90">{card.text}</p>
      </div>

      {/* stats */}
      {isUnit ? (
        <div className="absolute bottom-0.5 inset-x-1 flex items-center justify-center gap-1.5 z-10">
          ...
          {card.ranged && (
            <span className="flex items-center font-display font-bold text-[0.8rem] sm:text-base" title="A distancia: puede atacar al héroe" style={{ color: '#ffd76a' }}>
              <Sigil icon="bow" className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            </span>
          )}
          {(card.def ?? 0) > 0 && (
        ...
        <div className="absolute bottom-0.5 inset-x-1 text-center font-display text-[0.6rem] tracking-widest uppercase z-10" style={{ color: rarC }}>
          {card.spell?.school === 'heal' ? 'Apoyo' : card.spell?.school.startsWith('buff') || card.spell?.school === 'teamBuff' || card.spell?.school === 'warCry' ? 'Mejora' : 'Ítem'}
        </div>
      )}
    </button>
  );
